...
\begin{document}
	...
	\begin{itemize}
		...
		\[
		T_{Amdahl}(s, \gamma) = \underbrace{C_{startup}}_{\text{Fixed Overhead}} + \gamma \cdot \left((1 - p_{obs}) \cdot T(1)\right) + \gamma \cdot \left(\frac{p_{obs}}{s} \cdot T(1)\right)
		\]
	...
	\begin{enumerate}
		...
		Execute the workflow once with a moderate parallelism (e.g., $s=4$). Record the observed time $T_{obs}$ and the cluster load $L_{probe}$.
	...
	\begin{enumerate}
		\item \textbf{Model Retrieval:} Retrieve the specific parameters ($p_{obs}$, $N_{base}$, ...) and the (trained) GP from a registry (e.g. database, or similar)
		...
		\[
		P(s, \gamma, L_{cluster}) = \underbrace{C_{startup} + \gamma \cdot \left((1 - p_{obs}) \cdot T_{base}(1)\right) + \gamma \cdot \left(\frac{p_{obs}}{s} \cdot T_{base}(1)\right)}_{T_{Amdahl(s, \gamma)}} + \underbrace{GP(s, \gamma, L_{cluster})}_{R(\mathbf{x})}
		\]
